...
print(f"Here's the new list of invitees:{', '.join(guests)}.")#I'm using an f string, why are there still single quotes?: printing a string representation of the list--> displays the string as if it were a list
# solution: added another string( comma& space)--> has all the functions that strings have-->. join removes the items from the list and joins them and separates them by the string at the beginning (in this case the comman& space)
for guest in guests:
    print(f"Hey,{guest.title()} you have been invited to my party!")
print("I've found a larger table!")
new_guest2= guests.insert(0, "Lebron James") #Why "none": insert goes into a list and changes it in a place--> functions that belong to lists, job: modify the list
...
print(f"\t{guests}") # why can't I add functions like .strip()?--> Can only be applied to strings--> must convert to string before using
...
print(f"\tSadly, {cannot_attend.title()} will not be able to make it to the party.") # Message stating that the string assigned to cannot_attend (n this case , Einstein) will not be able to attend
new_guest= "George Washington"# define a new guest
print(f"{new_guest} has been invited to the party") #message stating that the string assigned to new_guest has been invited to the party
guests.append(new_guest)#use the append function to add new_guest to the end of the list
print(f"Here's the new list of invitees:{', '.join(guests)}.")##I'm using an f string, why are there still single quotes?: printing a string representation of the list--> displays the string as if it were a list
# solution: added another string( comma& space)--> has all the functions that strings have-->. join removes the items from the list and joins them and separates them by the string at the beginning (in this case the comma& space)
for guest in guests:
    print(f"Hey,{guest.title()} you have been invited to my party!")#using a for loop to print the same message for each of the guests
print("I've found a larger table!")
...
no_longer_invited4=guests.pop(1)#why out of range?--> every time the pop function is used, the index numbers are changed and you work with a modified list
...
print(f"{guests}")
del guests[:]# if nothing is specified to the left of the colon, it means start from the beginning, if nothing is specified to the right, it goes to the end. OR: use the clear function--> guests.clear()
